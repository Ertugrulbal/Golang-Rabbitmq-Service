...
func publishing(n int) (res int, err error) {
	...
	//Publishing a data and declaration of queue
	err = ch.Publish(
...
func publisher() {
	...
	for {
		...
		log.Printf(" [.] Point Store B is: %d", res)
...
func main() {
	//Main part of the Program A. We use a Go Routine function for the Consumer.
	go consumer()
	time.Sleep(3 * time.Second)
	publisher()
